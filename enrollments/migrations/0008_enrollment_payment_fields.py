from django.db import migrations, models

SQL_DROP_VIEW = "DROP VIEW IF EXISTS enrollments_salesrecord"

SQL_CREATE_VIEW = """
CREATE VIEW enrollments_salesrecord AS
SELECT
    e.id                                                        AS id,
    e.student_id                                                AS student_id,
    CASE
        WHEN TRIM(u.first_name || u.last_name) = ''
        THEN u.username
        ELSE TRIM(u.first_name || ' ' || u.last_name)
    END                                                         AS student_name,
    u.email                                                     AS student_email,
    b.course_id                                                 AS course_id,
    c.title                                                     AS course_title,
    c.price                                                     AS course_price,
    e.batch_id                                                  AS batch_id,
    b.name                                                      AS batch_name,
    b.company_id                                                AS company_id,
    comp.name                                                   AS company_name,
    e.status                                                    AS status,
    e.course_fee                                                AS course_fee,
    e.applied_discount                                          AS applied_discount,
    e.total_due                                                 AS total_due,
    e.payment_status                                            AS payment_status,
    e.request_date                                              AS request_date,
    e.approval_date                                             AS approval_date,
    e.certificate_given                                         AS certificate_given
FROM enrollments_enrollment  e
JOIN auth_customuser         u    ON e.student_id = u.id
JOIN enrollments_batch       b    ON e.batch_id   = b.id
JOIN courses_course          c    ON b.course_id  = c.id
JOIN accounts_company        comp ON b.company_id = comp.id
"""


class Migration(migrations.Migration):

    dependencies = [
        ('enrollments', '0007_enrollment_applied_discount_coupon_enrollment_coupon'),
    ]

    operations = [
        # Drop the existing view before touching the underlying table
        migrations.RunSQL(SQL_DROP_VIEW, reverse_sql=migrations.RunSQL.noop),

        migrations.AddField(
            model_name='enrollment',
            name='course_fee',
            field=models.DecimalField(
                decimal_places=2,
                default=0.00,
                max_digits=10,
                help_text='Snapshot of course price at enrollment time',
            ),
        ),
        migrations.AddField(
            model_name='enrollment',
            name='total_due',
            field=models.DecimalField(
                decimal_places=2,
                default=0.00,
                max_digits=10,
                help_text='course_fee minus applied_discount',
            ),
        ),
        migrations.AddField(
            model_name='enrollment',
            name='payment_status',
            field=models.CharField(
                choices=[
                    ('unpaid', 'Unpaid'),
                    ('paid', 'Paid'),
                    ('waived', 'Waived'),
                ],
                default='unpaid',
                max_length=20,
            ),
        ),

        # Recreate the view with the new columns
        migrations.RunSQL(SQL_CREATE_VIEW, reverse_sql=SQL_DROP_VIEW),
    ]
